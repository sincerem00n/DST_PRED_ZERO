...
class DSTTModelV2:
    def __init__(self, model_name='DSTTModel', early_stopping_patience=1):
        ...
        self.epistemics = None 
        self.aleatoric = None
        self.dates = None

        # GPU check inside __init__
        if tf.test.gpu_device_name() != '/device:GPU:0':
            print('WARNING: GPU device not found.')
        else:
    ...
    def build_base_model(self, input_shape, kl_weight=0.0001, dropout=0.4):
        input_shape = tf.TensorShape(input_shape)
        self.input = keras.Input(shape=input_shape)
        x = layers.Conv1D(filters=32, kernel_size=1, activation="relu",
                          name=f"{self.model_name}_conv",
                          kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                          bias_regularizer=regularizers.l2(1e-4),
                          activity_regularizer=regularizers.l2(1e-5))(self.input)
        x = layers.LSTM(650, return_sequences=True, name=f"{self.model_name}_lstm",
                        kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                        bias_regularizer=regularizers.l2(1e-4),
                        activity_regularizer=regularizers.l2(1e-5))(x)

        x = layers.Dropout(dropout)(x)
        x = layers.MultiHeadAttention(key_dim=4, num_heads=4, dropout=0.5, name=f"{self.model_name}_mh")(x, x)
        x = layers.Dropout(dropout)(x)
        x = layers.Dense(400, kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                         bias_regularizer=regularizers.l2(1e-4),
                         activity_regularizer=regularizers.l2(1e-5))(x)
        x = layers.Dropout(dropout)(x)

        print("Before DenseVariational, x.shape:", x.shape)
        # Reshape before DenseVariational
        x = layers.Flatten()(x)  # Or use Reshape((400,))
        # x = layers.Reshape((400,))(x)
        # x = tf.cast(x, dtype=tf.float32)
        print("After DenseVariational, x.shape:", x.shape)


        # x = tfp.layers.DenseVariational(100, self.posterior_mean_field, self.prior_trainable, kl_weight=kl_weight)(x)
        x = layers.Lambda(lambda y: tfp.layers.DenseVariational(100, self.posterior_mean_field, self.prior_trainable, kl_weight=kl_weight)(y), output_shape=(100,))(x)


        x = layers.Dropout(dropout)(x)
        x = layers.Dense(1, activity_regularizer=regularizers.l2(1e-5))(x)
    ...
    def fit(self, X_train, y_train, X_valid=None, y_valid=None, epochs=100, verbose=2, batch_size=2048):
    ...
    def predict(self, X_test, verbose=1):
        return np.squeeze(self.model.predict(X_test, verbose=verbose, batch_size=len(X_test)))

    def save_weights(self, w_dir="models"):
        weight_path = os.path.join(w_dir, "model_weights")
        os.makedirs(w_dir, exist_ok=True)
        self.model.save_weights(weight_path)

    def load_weights(self, w_dir="models"):
        weight_path = os.path.join(w_dir, "model_weights")
